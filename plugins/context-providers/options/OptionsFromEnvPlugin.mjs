...
const optionsFromEnvPlugin = async (passthrough) => {
	...
	options.set(
		publicUrlSymbol,
		Object.freeze(new URL(process.env.PUBLIC_URL ?? defaultPublicUrl.href)),
	);
	...
	options.set(
		listenPortSymbol,
		parseInt(process.env.LISTEN_PORT ?? (getPublicUrl().port || 80), 10),
	); // URL.port returns '' if using protocol default, so use || not ??

	// Local dev builds listen on a separate URL for HMR of the browser build
	options.set(
		hmrClientListenPortSymbol,
		parseInt(process.env.CLIENT_LISTEN_PORT ?? getListenPort() + 1, 10),
	);
	...
	options.set(
		hmrClientPublicUrlSymbol,
		Object.freeze(
			new URL(process.env.HMR_PUBLIC_URL ?? defaultHmrPublicUrl.href),
		),
	);

	options.set(
		shouldUseReactRefreshSymbol,
		stringToBoolean(process.env.SHOULD_USE_REACT_REFRESH ?? false),
	);

	options.set(
		shouldCalculateResourceIntegrity,
		stringToBoolean(process.env.RESOURCE_INTEGRITY ?? isProduction),
	);
	options.set(
		oldBrowserCompatibility,
		stringToBoolean(process.env.OLD_BROWSER_COMPAT ?? isProduction),
	);
	options.set(
		emotionSsr,
		stringToBoolean(process.env.EMOTION_SSR ?? isProduction),
	);
	options.set(
		debugStripped,
		stringToBoolean(process.env.DEBUG_STRIPPED ?? isProduction),
	);
	options.set(
		reactServerComponents,
		stringToBoolean(process.env.REACT_SERVER_COMPONENTS ?? false),
	);
	options.set(
		serverNodeArgs,
		JSON.parse(
			process.env.SERVER_NODE_ARGS ??
				JSON.stringify(["--inspect=0.0.0.0:9229"]),
		),
	);
	...
	options.set(
		minimizeNode,
		stringToBoolean(process.env.MINIMIZE_NODE ?? getMinimize()),
	);
	options.set(
		minimizeBrowser,
		stringToBoolean(process.env.MINIMIZE_BROWSER ?? getMinimize()),
	);
