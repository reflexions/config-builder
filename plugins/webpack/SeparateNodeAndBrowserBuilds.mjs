...
export const getIsNode = () =>
	targetPlatformContext.getStore() === targetPlatforms.node;
export const getIsBrowser = () =>
	targetPlatformContext.getStore() === targetPlatforms.browser;
...
const separateNodeAndBrowserBuilds = async () => {
	...
	const browserConfig =
		getBuildBrowser() &&
		(await targetPlatformContext.run(
			targetPlatforms.browser,
			async () => await getHookFnResult(buildBrowserConfig, () => null),
		));
	...
	const nodeConfig =
		...
		(await targetPlatformContext.run(
			...
			async () => await getHookFnResult(buildNodeConfig, () => null),
		));
...
const separateNodeAndBrowserBuildsCrumb = Symbol(
	separateNodeAndBrowserBuilds.name,
);
